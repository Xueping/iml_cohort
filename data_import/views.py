...
def process_file(f,new_features):
    ...
    file_path_data = os.path.join(settings.BASE_DIR, 'data/outcome_data.csv')
    ...
    new_data.to_csv(file_path_data,header=False)
...
def renew(request):
    if request.method == 'POST':
        ...
        file_path = os.path.join(settings.BASE_DIR, 'data/tmp.csv')
        process_file(file_path,new_features)

        return render(
            request,
            'data_import/selectboxs.html',
